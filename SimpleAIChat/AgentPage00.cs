...
namespace SimpleAIChat
{
    ...
    /// </summary>
    public partial class AgentPage01 : Form
    {
        // ---------------------------------------------------------
        // فیلدها
        // ---------------------------------------------------------
        ...
        private async void btnSend_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // ۶) تصمیم: ابزار یا چت معمولی؟
                string answer;

                bool useTools = _selectedPreset != null &&
                                string.Equals(_selectedPreset.OutputFormat, "tool", StringComparison.OrdinalIgnoreCase);

                if (useTools)
                {
                    // حالت ایجنت: با ابزارها
                    if (!_selectedModel.SupportsTools)
                    {
                        MessageBox.Show(
                            "این مدل از ابزارها پشتیبانی نمی‌کند.\n" +
                            "لطفاً یک مدل با SupportsTools = true انتخاب کنید.",
                            "Tool Not Supported",
                    ...
                    request.Tools = ToolRegistry.GetAll();

                    answer = await _llmService.SendWithToolsAsync(
                        request,
                ...
                {
                    Dictionary<string, string> parsed = TryParseJsonAnswer(answer, out string jsonError);
        ...
        /// </summary>
        private Dictionary<string, string> TryParseJsonAnswer(string raw, out string error)
        {
            error = null;
